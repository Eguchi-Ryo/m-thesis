...
axial_plane = 100  #zの位置を指定（体の高さ方向の位置）
sagittal_plane = 210 #xの位置を指定（体の左右方向の位置）
coronal_plane = 150 #yの位置を指定（体の奥行き方向の位置）
######################
...
def make_mask(wd):
    nrrd1_path = os.path.join(wd, "image", "new_3d_object_planes.nrrd")
    nrrd2_path = os.path.join(wd, "ans_data", "new_3d_object_planes.nrrd")
    ...
    # ?????????????????????????
    dilated_mask = binary_dilation(combined_mask, structure=structure, iterations=25)
...
def registration(wd):
    ...
    fx = os.path.join(wd, "ans_data", "new_3d_object_planes.nrrd")
    mv = os.path.join(wd, "image", "new_3d_object_planes.nrrd")
    param = os.path.join(wd, "registration", "param", "3D_elastix.txt")
